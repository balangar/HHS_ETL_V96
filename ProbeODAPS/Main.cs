            ...
            Dictionary<string, string> sfObjectQueries = new Dictionary<string, string>()
                {
                    {"Contact",
                        @"Select
	                        Id,
	                        IsDeleted,MasterRecordId,AccountId,LastName,FirstName,Salutation,MiddleName,Suffix,
	                        Name,RecordTypeId,
	                        OtherStreet,OtherCity,OtherState,OtherPostalCode,OtherCountry,OtherStateCode,OtherCountryCode,
                            OtherLatitude,OtherLongitude,OtherGeocodeAccuracy,
	                        OtherAddress,MailingStreet,
                            MailingCity,MailingState,MailingPostalCode,MailingCountry,MailingStateCode,MailingCountryCode,
	                        MailingLatitude,MailingLongitude,MailingGeocodeAccuracy,
	                        MailingAddress,
	                        Phone,Fax,MobilePhone,HomePhone,OtherPhone,AssistantPhone,
	                        ReportsToId,
	                        Email,Title,Department,AssistantName,
                            LeadSource,Birthdate,Description,OwnerId,HasOptedOutOfEmail,HasOptedOutOfFax,DoNotCall,
	                        CreatedDate,CreatedById,LastModifiedDate,LastModifiedById,SystemModstamp,
	                        LastActivityDate,LastCURequestDate,LastCUUpdateDate,LastViewedDate,LastReferencedDate,EmailBouncedReason,EmailBouncedDate,IsEmailBounced,
	                        PhotoUrl,Jigsaw,JigsawContactId,Additional_End_of_Life_Information__c,Agency__c,Alias__c,Approx_Age__c,Are_there_safety_issues_for_caseworkers__c,
	                        At_Risk_Population_Checkbox__c,At_Risk_Population__c,Best_Time_to_Reach__c,Biohazard__c,Bug_or_vermin_infestation__c,Burial_Cremation_Info__c,
	                        Client_Lives_With__c,Client_s_Home__c,Condition_Requiring_Total_Physical_Care__c,Contact_Record_System_ID__c,Contagious_disease__c,CountyPicklist__c,
	                        County_Held__c,Court_Report_Completed_Date__c,Court_Report_Due_Date__c,DNR__c,Dangerous_Aggressive_Person_in_Home__c,Dangerous_aggressive_animals__c,
	                        Dangerous_home_structure__c,Date_Assigned_to_County__c,Dementia_Alzheimer_s__c,Developmental_Intellectual_Disability__c,Ethnicity__c,
	                        Family_or_Other_Support__c,Firearms_or_other_weapons__c,Frail_Elderly__c,Gated_locked_community_building__c,Gender_Code__c,Gender__c,
	                        Health_Insurance__c,Hoarding_or_animal_hoarding__c,Illegal_drug_manufacturing__c,Illegal_drug_use__c,Inappropriate_Sexual_Behaviors__c,
	                        Income_Source__c,Istheclientatrisk__c,Kinship_Code__c,Known_Illegal_Activity__c,Known_criminal_activity_history__c,Known_gang_activity__c,
	                        Living_Will__c,Major_Mental_Illness_Emotional_Disorder__c,Marital_Status__c,Medicaid__c,Medically_Fragile__c,Medicare__c,
                            NAMRS_Client_Gender__c,NAMRS_Client_Language__c,NAMRS_Diagnosis_None__c,NAMRS_Perpetrator_Age__c,NAMRS_Perpetrator_Gender__c,
	                        NAMRS_Perpetrator_Race__c,NAMRS_Previous_Report__c,NAMRS_Race_Code__c,
	                        Neurological_Impairment_Stroke_TBI_MS__c,None__c,Organ_Donor__c,Other_Communnication_Needs__c,Other_Income_Source__c,
	                        Other_Insurance__c,Other_Relationship_to_Client__c,Perp_Lives_with_Client__c,Perpetrator_Age__c,Physical_Impairment__c,
	                        Police_frequently_called_to_the_home__c,Primary_Language__c,Professional_Support__c,Relationship_Category__c,Relationship_to_Client__c,
	                        Reporting_Party_Relationship_Notes__c,SSN__c,Safety_Notes__c,Safety_Warning__c,Security_system_cameras__c,Specify_Facility_Type__c,
	                        Specify_Illegal_Activity__c,Specify_Other_At_Risk_Info__c,Substance_Abuse_including_RX_meds__c,Translator_Needed_Other_Comm_Needs__c,
	                        Type_of_Authority__c,Type_of_Facility__c,Unknown_At_Risk_Population__c,Unsafe_neighborhood__c,Untreated_mental_illness__c,
	                        Work_Ext__c,Worker_Safety_Concerns__c,Worker_Safety_Issue__c,of_Cases__c,of_Intakes__c,Confidential_Reason__c,Confidential__c,
	                        Contact_Type__c,Current_City__c,Current_Postal_Code__c,Current_State_Code__c,Current_Street__c,Employment_Status__c,
	                        Other_Confidential_Reason__c,Person_Id__c,Race__c,Sexual_Orientation__c,Specify_Other_Primary_Language__c,
	                        Veteran_Status__c,Social_Security_Number__c,Bed_Bugs__c,Date_of_Death__c,County__c
                        From
	                        Contact"},
                    {"Case_Notes__C",
                        @"SELECT
